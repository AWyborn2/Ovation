// forked from design-sync lib/source-kit.mjs — synth entry appends explicit re-exports for componentSrcMap-pinned names so canonical implementations win star-export ambiguity
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // Explicit named re-exports win over ambiguous star exports (ESM
    // precedence): pin each componentSrcMap string entry to its canonical
    // module so duplicate names across the sweep resolve deterministically.
    const pinnedLines = Object.entries(srcMap)
      .filter(([k, v]) => typeof v === 'string' && /^[A-Z][A-Za-z0-9]*$/.test(k))
      .map(([k, v]) => `export { ${k} } from ${JSON.stringify(slash(resolve(PKG_DIR, v)))};`);
  ...
  if (synthEntry) {
    // Fork: srcMap pins must not preempt src-derived discovery — merge them.
    // Derived entries carry real group info, so they win for shared names.
    const derived = deriveComponentsFromSrc(srcFiles).filter((c) => srcMap[c.name] !== null);
    const dnames = new Set(derived.map((c) => c.name));
    components = [...derived, ...components.filter((c) => !dnames.has(c.name))];
  }
