...
| Script                    | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `playhq-project-fixtures` | Upserts each linked tenant's upcoming PlayHQ matches into `fixtures` (`source = 'playhq'`) for Social Studio; `--auto-link` links any unlinked tenant whose central club name matches exactly one PlayHQ organisation (the post-merge hook runs this on every deploy); `--tenant=<id> --set-org=<guid>` links one by hand. Needs `DATABASE_URL` + `CENTRAL_DATABASE_URL`; `--dry-run`, non-local hosts need `--yes`. `playhq-load --project` runs it after a load. |
