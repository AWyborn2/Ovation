...
- **Match history bulk load** — the master DB's complete PlayHQ-era match history (1,318 matches, 2013/14–2025/26, both innings) is loaded into `matches` / `match_player_lines` / `match_opposition_lines` via `scripts/src/load-matches.ts` (`pnpm --filter @workspace/scripts run load-matches [-- --commit]`). Picks the newest dump, builds a `staging` schema, previews by default. Runs `scripts/sql/matches-etl.sql`: maps master grade→app grade (PPL before 2019 → A Grade), season→start-year int, round/finals→`round`/`stage`; preserves master match identity in `source_key`; copies `opponent_club_id` from master clubs (#160, ids preserved). HH lines link `player_id` (NULL-player privacy-masked lines dropped); fielding not in master so 0 on lines (career fielding stays in baseline). Re-runnable (reversal phase keys on `source_key IS NOT NULL`). The step-7 season aggregation uses `DISTINCT ON` tables + indexes, NOT an O(n²) LATERAL, so the commit finishes; run detached via `nohup` since it exceeds the 120s bash cap.
