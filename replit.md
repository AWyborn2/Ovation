...
- **Duplicate-profile merges (`junior_participant_merges`, app-owned):** PlayHQ sometimes minted two participant GUIDs for the same child. Admins merge them permanently at `/admin/people/junior-players` (`POST /api/juniors/participants/{id}/merge`): every junior line is reassigned to the keeper, rosters deduped, senior link + privacy carried over, keeper metadata recomputed, the duplicate profile deleted, and the merge recorded in `junior_participant_merges` — which ETL step 7 re-applies after every full-replace reload (a merge whose keeper is missing from a new dump is skipped and reported loudly, never guessed). The absorbed GUID's profile URL aliases to the keeper. The map stays flat (no chains); using a merged-away GUID as keeper returns 409 with the canonical keeper.
...
- **Theming ("Broadcast", Sep 2026):** dark mode is the default (`ovation-theme` key; an explicit "system" follows the OS). Display headings and stat numbers use Barlow Condensed via `font-serif` (uppercase, 0.01em); body is IBM Plex Sans. Tenant colours flow through `deriveThemeTokens(brand, mode)` in `src/lib/theme-tokens.ts`; the static `:root`/`.dark` blocks in `index.css` are pinned by the theme-tokens sync test. Broadcast depth tokens: `--glass`, `--pop`, `--glow`, `--shadow-pop`, `--win-*`/`--loss-*`, `--header-h`, `--pad`, `--gap-section`; `nav:` (~900px) is the desktop-header breakpoint. Accent _text_ is `text-primary-text` (contrast-safe `--primary-text`, ≥4.5:1) — a bare `text-primary` is blocked by a guard test; on always-dark surfaces use `text-[hsl(var(--primary))]`. Shared primitives live in `src/components/broadcast/` (PageHero, PageHeader, SectionCard, StatTile, TableCard, UnderlineTabs, …); full-bleed pages mark their root with `<Container page>`/`<FullBleedPage>`. Tenant hero/explore photos come from `tenants.hero_images` (`useHeroImage`), falling back to a brand gradient. See `.design-sync/conventions.md`.
...
- **Match Detail** (`/matches/:id`) — Broadcast two-innings scorecard (`components/scorecard/broadcast-scorecard.tsx`). Innings render in true batting order from `matches.hhccBattedFirst`; each is a batting card + bowling card with dismissal text, strike-rate, overs (ball notation), economy, extras breakdown. HH players (playerId < 90000) are tappable → career-stats popup; opposition and fill-ins render as plain text. View-model is the shared lib `@workspace/scorecard` (`buildScorecard`), reused by web + mobile. Hat-tricks show a flame badge (admin toggle panel, web only). Opposition brand colours from `clubs.primaryColour/secondaryColour`; missing logo degrades to an initials chip.
