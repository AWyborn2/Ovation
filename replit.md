...
Junior data lives in isolated `junior_*` tables and is served only via `/api/juniors/*`. No junior query ever touches a senior table; junior and senior stats NEVER combine. The only bridge is `junior_participants.senior_player_id` (a nullable FK to `players.id`) used purely as a **cross-reference link**, never to merge figures.

- **junior_matches** — one row per junior match (PlayHQ export). `is_halls_head` semantics live on the lines; the match row carries `team1/team2` + `team1_score/team2_score`, `opponent_name`, `hh_result`, `hh_batted_first`, plus messy free-text `season` ("2024/25"), `age_group`, `round`, often-null `match_date`/scores, and statuses like "Played (stats not recorded)" / "No Result".
