...
- **Committee & Captains** — office bearers (Committee tab on Honour Boards) and grade captains (per grade), from `club_roles`. Admin CRUD at `/admin/committee` (publish toggle); names link to players where confident. Seeded from `artifacts/api-server/src/data/club-roles.json` via `scripts/src/seed-committee.ts`.
