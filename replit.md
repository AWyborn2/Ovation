...
- **junior_matches** — one row per junior match (PlayHQ export). `is_halls_head` semantics live on the lines; the match row carries `team1/team2` + `team1_score/team2_score`, `opponent_name`, `hh_result`, `hh_batted_first`, plus messy free-text `season` ("2024/25"), `age_group`, `round`, often-null `match_date`/scores, and statuses like "Played (stats not recorded)" / "No Result". The ETL **normalizes** `age_group` to a canonical token (`pg_temp.jr_norm_age` maps "Under 14"/"U14"→"U14", "Year 9 Boys Peel"→"Year 9", "Peel Girls League"→"Girls League"; prefers the dump's clean `age_group`, falls back to deriving from `grade`) and parses `season_start_year` (int) from the season string for reliable newest-first ordering.
