...
function MatchLineRow({ m }: { m: PlayerMatchLine }) {
  const colors = useColors();
  const fieldParts = [
  ...
  return (
    <Link href={`/matches/${m.matchId}` as never} asChild>
...
export default function PlayerDetailScreen() {
  ...
  const hasMatchLines = !!matchLines && matchLines.length > 0;
