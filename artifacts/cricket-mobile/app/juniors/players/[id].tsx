...
function JuniorMatchRow({ m }: { m: JuniorPlayerMatchLine }) {
  const colors = useColors();
  const batNotOut = m.batting?.dismissal && /not out/i.test(m.batting.dismissal);
  return (
    <Link href={`/juniors/matches/${m.matchId}` as never} asChild>
  ...
  return (
    <>
      ...
      <FlatList
        ...
        data={hasMatches ? player.matches : []}
        keyExtractor={(m) => String(m.matchId)}
        renderItem={({ item }) => <JuniorMatchRow m={item} />}
        ListHeaderComponent={
          <>
            <Body
              bold
