...
// B20 — Club Leaders · Wickets (clubLeaderboard · Wickets preset). Leading
// wicket-taker for every grade, one card. Same layout as B19 (see
// club-leaderboard-runs.ts for the transcription notes) with the category
// swapped and the bundle's slightly smaller foil headline (the longer "CLUB
// WICKETS" sets at 80px where "CLUB RUNS" sets at 86px).
//
// Note: despite this being the card the bundle's DC script sizes for all three
// formats, its markup carries NO isNotStory branch — the layout is the same
// single fluid column as the rest of the pack, so it follows the standard
// story + shared shape here.
...
const storyHtml = columnRoot(
  ...
  centredStoryHeader("{{subtitle}}") +
    ...
    foilText("{{title}}", 80, ";font-size:calc(var(--k,1.4)*80px);line-height:.86") +
    `</div>` +
