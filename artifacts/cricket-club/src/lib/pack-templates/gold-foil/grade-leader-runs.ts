...
// B13 — Leaderboard (gradeLeader · Runs preset). Grade category leader with
// the big foil number. B18 is the same layout with the Wickets preset.
//
// The bundle ships ONE fluid layout for this card; the story format is the
// transcription, the shared format is authored from Pack A grade-leader-runs'
// shared structure. Field keys are IDENTICAL to Pack A's (R4 parity), which is
// also why the bundle's three-logo sponsor strip becomes the presented-by
// line — Pack A's gradeLeader declares no sponsor1..3 keys.
...
// (differs from fragments' storyHeader, which has no logo slot).
...
const storyHtml = columnRoot(
  ...
  centredStoryHeader("{{grade}} LEADERBOARD") +
    ...
    `<div style="font:600 17px/1 ui-monospace,Menlo,monospace;letter-spacing:.22em;color:rgba(255,255,255,.62);margin-top:14px">LEADING RUN-SCORER</div></div>` +
    ...
    // Bundle literal "RUNS · 2025/26" — the category/season pair.
