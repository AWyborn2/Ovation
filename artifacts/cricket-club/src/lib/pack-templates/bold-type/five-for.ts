...
// C17 — Five-for. Giant figures, framed bowler photo, gold name.
//
...
// {{grade}} sits in the shared meta line and {{wickets}} in the shared header
// tag ("{{wickets}} WICKETS"), as Pack A composes them, so every declared key
// stays bound.
//
...
// "FIVE-WICKET HAUL" left and "FIVE-FOR" right; storyHeader has one tag slot,
// so the tag composes both. The bundle's static sub-line ("RIPPED THROUGH THE
// ORDER") is club-neutral and kept verbatim. The shared layout is authored
// from Pack A's shared structure (figures column left, photo right) in Bold
// Type's language.
...
// ---------------------------------------------------------------------------
// Story (1080×1920) — giant figures, framed photo, gold name, gold bar
// ---------------------------------------------------------------------------

const storyHtml = storyColumnRoot(
  storyHeader("FIVE-WICKET HAUL · FIVE-FOR") +
    `<div style="flex:1;min-height:0;display:flex;flex-direction:column;gap:20px;margin-top:22px">` +
    `<div><div style="font-family:var(--disp,'Anton'),sans-serif;font-size:232px;line-height:.78;text-transform:uppercase;color:#fff">{{figures}}</div>` +
    `<div style="font:500 19px/1.3 ui-monospace,Menlo,monospace;letter-spacing:.12em;color:rgba(255,255,255,.5);margin-top:14px;text-transform:uppercase">{{overs}} OVERS · vs {{opponent}} · RD {{round}}</div></div>` +
    STORY_PHOTO +
    ...
    `<div style="font:500 19px/1.3 ui-monospace,Menlo,monospace;letter-spacing:.12em;color:rgba(255,255,255,.5);margin-top:8px">RIPPED THROUGH THE ORDER</div></div>` +
    `</div>` +
...
const sharedHtml = sharedColumnRoot(
  sharedHeader("FIVE-FOR", "{{wickets}} WICKETS") +
    ...
    `<div style="font:700 20px/1 ui-monospace,Menlo,monospace;letter-spacing:.26em;color:var(--gold,#FBAC27)">FIVE-WICKET HAUL</div>` +
    `<div style="font-family:var(--disp,'Anton'),sans-serif;font-size:calc(var(--k,1.4)*150px);line-height:.82;color:#fff"><span>{{figures}}</span><span style="font-size:calc(var(--k,1.4)*44px);color:rgba(255,255,255,.6)"> ({{overs}})</span></div>` +
