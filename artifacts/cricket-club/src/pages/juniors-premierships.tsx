    ...
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-serif font-bold text-primary flex items-center gap-2">
          <Crown className="h-7 w-7 text-emerald-600" /> Junior Premierships
        </h1>
      ...
      {ageGroups.length > 0 && (
        <div className="flex flex-col gap-1 max-w-xs">
          ...
          <select
            ...
            className="px-3 py-2 rounded border-2 border-emerald-600 bg-card text-foreground text-sm font-medium"
          ...
          {filtered.map((p) => (
            <div key={p.id} className="bg-card border border-border rounded-md shadow-sm overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                ...
                {p.players.length > 0 && (
                  <div>
                    <div className="text-xs font-bold uppercase tracking-widest text-emerald-700 mb-1">Roster</div>
                    <div className="flex flex-wrap gap-1.5">
                      {p.players.map((pl, i) =>
                        pl.participantId ? (
                          <Link key={i} href={`/juniors/players/${pl.participantId}`}>
                            <span className={`text-xs rounded-full border ${JUNIOR_ACCENT.borderSoft} px-2 py-0.5 cursor-pointer hover:bg-emerald-500/10`}>
                              {pl.playerName}
                            </span>
                          </Link>
                        ) : (
