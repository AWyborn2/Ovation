...
function Editor({ brand }: { brand: TenantBrand }) {
  ...
  const [customSecondary, setCustomSecondary] = useState(
    brand.primaryColour ??
      ACCENT_HEX[snapHexToAccentToken(brand.primaryColour ?? brand.backgroundColour)],
  );
  ...
  const handleLogoFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (palette.backgroundColour) {
        const suggested = snapHexToAccentToken(palette.primaryColour ?? palette.backgroundColour);
        setAccent(suggested);
        setColourNote(
  ...
  const themeOverridesPayload = Object.keys(overrides).length > 0 ? overrides : null;
  ...
  const previewSecondary = colourTab === "preset" ? ACCENT_HEX[accent] : customSecondary;
  const previewBrand = {
    ...brand,
    name,
    logoUrl: logoUrl || null,
      ...
      <div className="space-y-6 xl:col-span-2">
        ...
        <div className="gap-6 sm:columns-2 [&>*]:mb-6 [&>*]:break-inside-avoid">
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Choose the overall look of your site. You can still set your accent colour and
                fine-tune below either way.
              </p>
              <div className="grid gap-3 sm:grid-cols-2" role="radiogroup" aria-label="App look">
                {APP_LOOKS.map((look) => {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Your club's colours — used for buttons, highlights, and every number that matters.
              </p>
              ...
              {colourTab === "preset" ? (
                ...
                <div className="space-y-3">
                  ...
                  />
                  {contrastWarning && (
                    <p className="text-sm text-amber-600 dark:text-amber-500">{contrastWarning}</p>
                  )}
                </div>
              )}
            </CardContent>
          ...
          <Card>
            ...
            {advancedOpen && (
              <CardContent className="space-y-5">
                <p className="text-sm text-muted-foreground">
                  Fine-tune individual surfaces and text colours. Each is derived automatically from
                  your colours until you set it — leave on <em>Auto</em> for the guardrailed
                  default. Text colours invert automatically between light and dark mode so they
                  stay readable.
                </p>
