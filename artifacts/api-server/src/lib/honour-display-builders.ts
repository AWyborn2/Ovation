/**
 * Honour-display board builders.
 *
 * Extracted from routes/honour-display.ts, where 1,823 of 1,941 lines were
 * board assembly, grid cataloguing, brand resolution and kiosk-token
 * handling, with only ~118 lines of route handlers on the end — the router
 * was a thin shell around a builder library that happened to share its file.
 *
 * Depends only on the db layer, settings and request context, never on a
 * route, so importing these back into the router cannot create a cycle.
 * Same shared-core-first pattern as lib/junior-admin-helpers.ts.
 */
...
import { buildMilestonesForRequest } from "../routes/milestones";
...
export async function buildMilestoneBoard(req: Request): Promise<HonourBoardOut | null> {
