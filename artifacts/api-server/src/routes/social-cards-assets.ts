...
import { db, sponsorsTable, cardThemesTable, cardAudioTracksTable } from "@workspace/db";
import {
  CreateSponsorBody,
  UpdateSponsorBody,
  UpdateSponsorParams,
...
router.delete(
  ...
  async (req, res): Promise<void> => {
    ...
    const result = await db
      ...
      .where(
        and(eq(sponsorsTable.id, params.data.id), eq(sponsorsTable.tenantId, getTenantId(req))),
      )
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    const tenantId = getTenantId(req);
    const scoped = and(
      eq(cardThemesTable.id, params.data.id),
      eq(cardThemesTable.tenantId, tenantId),
    );
    const row = await db.transaction(async (tx) => {
