...
import { signupRateLimiter, signupDiscoveryRateLimiter } from "../middlewares/rate-limit";
...
router.post("/platform/signup", signupRateLimiter, async (req, res): Promise<void> => {
  if (signupMode() === "off") {
    res.status(403).json({ error: "Signup is disabled" });
    return;
  }
