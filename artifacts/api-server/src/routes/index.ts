import { Router, type IRouter } from "express";
import healthRouter from "./health";
import authRouter from "./auth";
import captainAuthRouter from "./captain-auth";
import adminsRouter from "./admins";
import captainsRouter from "./captains";
import awardVotingRouter from "./award-voting";
import awardPointsRouter from "./award-points";
import playersRouter from "./players";
import playerCurationRouter from "./player-curation";
import statsRouter from "./stats";
import gradesRouter from "./grades";
import premiershipsRouter from "./premierships";
import importsRouter from "./imports";
import matchesRouter from "./matches";
import milestonesRouter from "./milestones";
import capsRouter from "./caps";
import lifeMembersRouter from "./life-members";
import honourBoardsRouter from "./honour-boards";
import awardsRouter from "./awards";
import teamOfDecadeRouter from "./team-of-decade";
import clubRolesRouter from "./club-roles";
import peopleRouter from "./people";
import recordsRouter from "./records";
import historicalRouter from "./historical";
import socialCardsRouter from "./social-cards";
import socialPrefillRouter from "./social-prefill";
import fixturesRouter from "./fixtures";
import fixturesResultsRouter from "./fixtures-results";
import socialDraftsRouter from "./social-drafts";
import clubPhotosRouter from "./club-photos";
import notificationsRouter from "./notifications";
import postPackRouter from "./post-pack";
import cardVideoRouter from "./card-video";
import juniorsRouter from "./juniors";
import juniorsAdminRouter from "./juniors-admin";
import navItemsRouter from "./nav-items";
import storageRouter from "./storage";
import platformStorageRouter from "./platform-storage";
import tradingCardSettingsRouter from "./trading-card-settings";
import tourContentRouter from "./tour-content";
import honourDisplayRouter from "./honour-display";
import tenantRouter from "./tenant";
import platformRouter from "./platform";
import platformAdminRouter from "./platform-admin";
import billingRouter from "./billing";

const router: IRouter = Router();

router.use(healthRouter);
router.use(authRouter);
router.use(captainAuthRouter);
router.use(adminsRouter);
router.use(captainsRouter);
router.use(awardVotingRouter);
router.use(awardPointsRouter);
router.use(playersRouter);
router.use(playerCurationRouter);
router.use(statsRouter);
router.use(gradesRouter);
router.use(premiershipsRouter);
router.use(importsRouter);
router.use(matchesRouter);
router.use(milestonesRouter);
router.use(capsRouter);
router.use(lifeMembersRouter);
router.use(honourBoardsRouter);
router.use(awardsRouter);
router.use(teamOfDecadeRouter);
router.use(clubRolesRouter);
router.use(peopleRouter);
router.use(recordsRouter);
router.use(historicalRouter);
router.use(socialCardsRouter);
router.use(socialPrefillRouter);
router.use(fixturesRouter);
router.use(fixturesResultsRouter);
router.use(socialDraftsRouter);
router.use(clubPhotosRouter);
router.use(notificationsRouter);
router.use(postPackRouter);
router.use(cardVideoRouter);
router.use(juniorsRouter);
router.use(juniorsAdminRouter);
router.use(navItemsRouter);
router.use(storageRouter);
router.use(tradingCardSettingsRouter);
router.use(tourContentRouter);
router.use(honourDisplayRouter);
router.use(tenantRouter);
router.use(platformRouter);
router.use(platformAdminRouter);
router.use(platformStorageRouter);
router.use(billingRouter);

export default router;
