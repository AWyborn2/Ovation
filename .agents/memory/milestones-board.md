...
**The window is AUTHORITATIVE when any match is dated** (`windowStart != null`): the route returns ONLY in-window items, most-recent-first, and `featured = items.length > 0`. It falls back to all-time-by-significance ONLY when no match is dated at all. (Older behavior — "≥5 distinct players → feature recent then show the rest by significance" — was replaced because it made the recency setting feel ignored: setting "last week" still showed 2023 entries.)
