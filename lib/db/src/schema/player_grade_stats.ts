...
export const playerGradeStatsTable = pgTable(
  "player_grade_stats",
  {
  ...
  (t) => ({
    idxPlayer: index("player_grade_stats_player_idx").on(t.playerId),
    idxGrade: index("player_grade_stats_grade_idx").on(t.grade),
  }),
);

export const insertPlayerGradeStatSchema = createInsertSchema(playerGradeStatsTable).omit({
  id: true,
});
