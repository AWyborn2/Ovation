...
// website Matches page behaves by default: which grade + season load first, the
// order grades appear in the grade menu, and the within-season round-order
// direction. App-config (never replaced by the master ETL).
export const matchDisplaySettingsTable = pgTable(
  "match_display_settings",
  {
    ...
    // Ordered list of grade names for the grade menu. Grades not listed fall
    // back to the built-in seniority order, appended after the configured ones.
    gradeOrder: text("grade_order").array().notNull().default([]),
    // Within-season round direction: "desc" (latest round first, current
    // default) or "asc" (round 1 first). Season ordering always stays newest-first.
    roundOrder: text("round_order").notNull().default("desc"),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => ({
    uniqTenant: uniqueIndex("match_display_settings_tenant_unique").on(t.tenantId),
  }),
);
