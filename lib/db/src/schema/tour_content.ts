...
// One row per tenant (unique on tenantId) holding admin-editable onboarding
// copy: the first-visit welcome dialog title/body and the per-step
// titles/descriptions for the fan and admin guided tours. Stored as overrides
// only — anything left blank falls back to the in-code defaults, so the code
// remains the single source of truth for tour *structure* (which sections are
// highlighted) while clubs can re-word the copy without a developer. App-config
// (never replaced by the master ETL).
export const tourContentTable = pgTable(
  "tour_content",
  {
  ...
  (t) => ({
    uniqTenant: uniqueIndex("tour_content_tenant_unique").on(t.tenantId),
  }),
);
