...
/**
 * Honour-board maths shared by the website and the mobile app.
 *
 * Everything in here is pure: no React, DOM or React Native. Each app keeps a
 * thin wrapper (`artifacts/cricket-club/src/lib/honour-boards.ts`,
 * `artifacts/cricket-mobile/lib/honour-boards.ts`) that re-exports this module
 * and adds its platform hooks. Tier thresholds are club policy and live here
 * ONCE as `HONOUR_TIER_CONFIG` so web and mobile can never drift apart again.
 */
...
/** Admin-editable subset of `BoardMeta` as returned by `GET /honour-boards`. */
export interface BoardMetaOverride {
  key: string;
  label?: string | null;
  title?: string | null;
...
/**
 * Merges admin overrides on top of the static defaults so label/title/subtitle
 * edits show up on the public surfaces. Board order and membership are fixed
 * by `defaults`; overrides for unknown keys are ignored (the boards' maths
 * only knows the `BoardKey` union). Returns `defaults` itself when there are
 * no overrides so callers can rely on referential stability.
 */
export function mergeBoardsMeta(
  overrides: readonly BoardMetaOverride[] | null | undefined,
  defaults: BoardMeta[] = DEFAULT_BOARDS,
): BoardMeta[] {
  if (!overrides?.length) return defaults;
  const byKey = new Map(overrides.map((b) => [b.key, b]));
...
/** One membership band of a board, e.g. "100 Games Club" = 100..149. */
export type TierDef = { label: string; min: number; max?: number };

/**
 * How a board's tiers are produced. `static` boards list their bands outright.
 * `extendable` boards have a fixed ladder below `anchorMin` and grow upwards in
 * `step`s so the top band always covers the club's current record holder.
 */
...
/**
 * CLUB POLICY — the tier thresholds for every honour board. This is the only
 * copy; web and mobile both read it from `@workspace/scorecard`.
 */
export const HONOUR_TIER_CONFIG: Record<BoardKey, TierConfig> = {
  games: {
    kind: "extendable",
    noun: "Games",
    step: 50,
...
/**
 * Tiers for a board given the players on it — extendable boards grow their top
 * band to cover the highest value present (see `TierConfig`).
 */
export const buildTiers = (key: BoardKey, players: AggregatedPlayer[]): TierDef[] => {
  const cfg = HONOUR_TIER_CONFIG[key];
