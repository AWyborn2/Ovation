-- ============================================================================
--  Halls Head CC — innings-order fix  (apply to match_batting / match_bowling)
--  Generated 2026-06-05
--
--  WHAT THIS DOES
--    Normalises the `innings` column to a consistent 1-based batting sequence:
--      innings = 1  -> the side that BATTED FIRST
--      innings = 2  -> the side that batted second
--    Legacy PlayHQ matches (2013/14–2022/23) were stored 0-based ({0,1});
--    2023/24+ matches were already 1-based ({1,2}). This shifts only the
--    legacy 0-based matches by +1 in BOTH tables, leaving the rest untouched.
--    After this runs, order a scorecard's two innings by `innings` instead of
--    assuming Halls Head bats first.
--
--  SCOPE
--    939 matches renumbered  (21512 match_batting rows, 12662 match_bowling rows).
--    379 already-1-based matches are left unchanged.
--
--  SAFE TO RE-RUN (idempotent): it only touches matches whose lowest innings
--    is still 0, so a second run is a no-op. Runs as one transaction.
--
--  ENGINE: written for PostgreSQL (also valid in SQLite). If your app keeps the
--    innings number on different table/column names, change the 4 identifiers
--    below (match_batting, match_bowling, innings, match_id) to match.
-- ============================================================================

BEGIN;

-- Snapshot the legacy (0-based) matches BEFORE updating, using the lowest
-- innings across BOTH cards so batting and bowling stay aligned.
CREATE TEMPORARY TABLE _hh_legacy_innings AS
  SELECT match_id
  FROM (
    SELECT match_id, innings FROM match_batting
    UNION ALL
    SELECT match_id, innings FROM match_bowling
  ) AS all_innings
  GROUP BY match_id
  HAVING MIN(innings) = 0;

UPDATE match_batting
  SET innings = innings + 1
  WHERE match_id IN (SELECT match_id FROM _hh_legacy_innings);

UPDATE match_bowling
  SET innings = innings + 1
  WHERE match_id IN (SELECT match_id FROM _hh_legacy_innings);

DROP TABLE _hh_legacy_innings;

COMMIT;

-- ----------------------------------------------------------------------------
-- VERIFICATION (run after COMMIT — every row should report OK / 0)
-- ----------------------------------------------------------------------------
-- 1) Every match's innings now sit in {1,2}:
--   SELECT COUNT(*) AS matches_out_of_range FROM (
--     SELECT match_id FROM (
--       SELECT match_id,innings FROM match_batting
--       UNION ALL SELECT match_id,innings FROM match_bowling
--     ) g GROUP BY match_id HAVING MIN(innings) <> 1 OR MAX(innings) > 2
--   ) x;                                         -- expect 0
--
-- 2) No physical innings has the same team batting and bowling:
--   SELECT COUNT(*) AS same_team_innings
--   FROM match_batting b JOIN match_bowling w
--     ON b.match_id=w.match_id AND b.innings=w.innings
--    AND b.batting_team=w.bowling_team;          -- expect 0
--
-- 3) Halls Head bowled first (innings 1 = opposition) in ~562 matches:
--   SELECT COUNT(DISTINCT match_id) AS hh_bowled_first
--   FROM match_batting WHERE innings=1 AND is_halls_head=0;   -- expect 562
-- ----------------------------------------------------------------------------

-- Audit trail: the 939 match_id values renumbered by this script
-- (matches the CSV column innings_renumbered = 'yes'):
/*
    ...
    1099, 1100, 1101, 1102, 1103, 1104, 1105, 1106, 1107, 1108, 1109, 1110, 1111, 1112, 1113, 1114, 1116, 1117, 1118, 1119,
