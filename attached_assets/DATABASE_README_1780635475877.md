...
| `matches` | One row per match (**2003/04–2025/26**, 2,102 matches): season, short `grade` code (A, B, … for grouping), `competition` (full grade descriptor e.g. "A Grade: Wyllie Cup"), round, date, venue, both teams + scores, plus `playhq_match_id` (stable PlayHQ match GUID) and `hh_team_id` (PlayHQ season team GUID). The Halls Head side is labelled **HHCC** in `hh_team`/`team1`/`team2` (and in `batting_team`/`bowling_team`); opponents keep their real name where available |
...
### Scorecards & fill-ins
- `matches`/`match_batting`/`match_bowling` now cover **2003/04–2025/26** (2,102 matches) for every Halls Head team (A–F, U21 Colts, Female A, Female B, PPL, Mid-Year T20). This is the full available play.cricket.com.au / PlayHQ history.
